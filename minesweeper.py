...
class Board:
    ...
    def make_new_board(self):
        # construct a new board based on the dim size and numbers
        ...
        while bombs_planted < self.num_bombs:
            ...
            col = loc % self.dim_size  # we want the remember to tell us what index in that row to loop
    ...
    def __str__(self):
        ...
        # return a string that shows the bard to the player
...
def play(dim_size=10, num_bombs=10):
    ...
    #           next to a bomb
    # Step 4: repeat steps 2 and 3a/b until there are no more pleases to dig -> VICTORY!
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        ...
        # if it's a valid, we dig
        safe = board.dig(row, col)
        if not safe:
            # dug a bomb ahhhhh
            break  # game over rip

    # 2 way to end loop, lets check which one
    if safe:
